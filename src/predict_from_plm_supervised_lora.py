...
def get_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Predict labels using a supervised PLM checkpoint (LoRA adapters + classifier head)."
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
        required=True,
        help="Path to a checkpoint directory (e.g., .../pLM_encoder_1000_supervised/checkpoint-1363).",
    )
...
def _load_label_info(base_dir: str) -> Dict[str, Any]:
...
def _load_classifier_head(base_dir: str, hidden_size: int, num_labels: int) -> PredictionHead:
    head_path = os.path.join(base_dir, "classifier_head.pt")
...
def main(config: Dict[str, Any]) -> None:
    ...
    if not os.path.isdir(checkpoint):
        raise ValueError(f"Invalid checkpoint path: '{checkpoint}'. Expected a directory.")

    base_dir = os.path.abspath(os.path.join(checkpoint, os.pardir))
    label_info = _load_label_info(base_dir)
    ...
    if num_labels == 0:
        raise ValueError("label_mapping.json does not contain a valid id2label mapping.")
    ...
    try:
        ...
        raise ImportError("The 'peft' package is required to load LoRA adapters.") from exc

    print(f"Loading LoRA adapters from: {checkpoint}")
    model = PeftModel.from_pretrained(model, checkpoint)

    hidden_size = int(model.config.hidden_size)
    head = _load_classifier_head(base_dir, hidden_size=hidden_size, num_labels=num_labels)
